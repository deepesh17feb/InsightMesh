...
def new_trace(spec_id: str = "", run_mode: str | None = None) -> str:
    """Return the active trace id, minting one if called outside a trace() block.
    spec_id/run_mode are accepted for caller compatibility; the trace name and
    tags are set by trace() itself."""
    global _current_trace_id, _current_trace_url
    if _current_trace_id:
        return _current_trace_id
    try:
        c = client()
        _current_trace_id = c.get_current_trace_id() or c.create_trace_id()
